...
tokenizer = AutoTokenizer.from_pretrained(model_path, trust_remote_code=True)  # 导入chatglm 你可以换你喜欢的版本模型
#quantize量化配置；
...
#lora加载训练模型
model = PeftModel.from_pretrained(model, "LLaMA-Factory/saves/ChatGLM2-6B-Chat/lora/yinmei-20231123-ok-last")
...
def ai_response():
    ...
    if len(history) >= len(Role_history)+history_count and enable_history:  # 如果启用记忆且达到最大记忆长度
        ...
        print(response)
    answer = f'回复{user_name}：唧唧  {response}'
    #加入回复列表，并且后续合成语音
